...
class PetitionProvider with ChangeNotifier {
  ...
  Future<Petition?> fetchPetitionByCaseId(String caseId) async {
    try {
      ...
      debugPrint(
          '📂 CaseId not found in online petitions. Searching offlinepetitions...');
    ...
    try {
      ...
      if (updates.containsKey('policeStatus')) {
        try {
          ...
          const backendUrl =
              'https://fastapi-app-335340524683.asia-south1.run.app'; // Citizen backend URL

          debugPrint(
              '📡 [PETITION_UPDATE] Calling backend to trigger notification...');
          debugPrint(
              '📡 URL: $backendUrl/api/petitions/$petitionId/update-status');
          debugPrint(
              '📡 Old status: $oldPoliceStatus → New status: ${updates['policeStatus']}');
          ...
          if (response.statusCode == 200) {
            ...
            debugPrint(
                '📱 Notification sent: ${response.data['notificationSent']}');
          } else {
            debugPrint(
                '⚠️ [PETITION_UPDATE] Backend returned ${response.statusCode}');
          }
        } catch (apiError) {
          // Don't fail the whole operation if notification fails
          debugPrint(
              '⚠️ [PETITION_UPDATE] Failed to notify backend: $apiError');
          debugPrint(
              '📝 Firestore update succeeded, but notification may not have been sent');
        }
      }
  ...
  Future<void> fetchFilteredPetitions({
    ...
    try {
      debugPrint(
          '🔍 fetchFilteredPetitions | isPolice=$isPolice | officerId=$officerId | station=$stationName | district=$district | range=$range | filter=$filter');
      ...
      debugPrint(
          '✅ fetchFilteredPetitions found: ${_petitions.length} petitions');
    } catch (e) {
  ...
  Future<bool> createPetitionUpdate({
    ...
    try {
      debugPrint(
          '🚀 [PETITION_UPDATE] Creating update for petition: $petitionId');
      ...
      if (photoFiles != null && photoFiles.isNotEmpty) {
        try {
          debugPrint(
              '📸 [PETITION_UPDATE] Uploading ${photoFiles.length} photos');
          final timestamp = DateTime.now()
              .toString()
              .split('.')
              .first
          ...
          debugPrint(
              '✅ [PETITION_UPDATE] Photos uploaded: ${photoUrls.length} URLs');
        } catch (photoError) {
      ...
      if (documentFiles != null && documentFiles.isNotEmpty) {
        try {
          debugPrint(
              '📄 [PETITION_UPDATE] Uploading ${documentFiles.length} documents');
          final timestamp = DateTime.now()
              .toString()
              .split('.')
              .first
          ...
          debugPrint(
              '✅ [PETITION_UPDATE] Documents uploaded: ${documents.length} with URLs');
        } catch (docError) {
      ...
      try {
        ...
        if (userId != null) {
          ...
          debugPrint(
              '📲 [PETITION_UPDATE] Notifying citizen about case update...');
        ...
        debugPrint(
            '⚠️ [PETITION_UPDATE] Notification failed (continuing): $notifError');
        // Don't fail the whole operation if notification fails
      }
